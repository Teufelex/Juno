...
class JunoTours extends React.PureComponent {
    ...
    render() {
        if (this.state.activeElem) 
            return (
                <div className="Tours">
                <JunoTour tour={this.state.activeElem}  key={this.state.activeElem.code} act={true}/>
                </div>
            );
        let toursArr = [];
        let pagesArr = [];
        ...
        showElem.forEach(b => {
            let block = <JunoTour tour={b}  key={b.code}/>;
            toursArr.push(block);
        });
    
        for(let i = 1; i <= numOfPages; i++) {
            if (i !== 1) pagesArr.push(<div key={i + 10}>|</div>);
            pagesArr.push(<NavLink to={"/tours/" + i} key={i}>{i}</NavLink>);
        }
